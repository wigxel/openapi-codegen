...
import type { ConfigBase, Context } from "./types";
import type { OperationObject, PathItemObject } from "openapi3-ts";
...
export type Config = ConfigBase & {
  ...
  /**
   * Generate Vue Query components with `useSuspenseQuery` hook.
   *
   * @default false
   */
...
export const generateVueQueryComponents = async (context: Context, config: Config) => {
    ...
    nodes
      .map((node: ts.Node, i, nodes) => {
        return (
          ...
          (ts.isJSDoc(node) ||
            (ts.isImportDeclaration(node) && nodes[i + 1] && ts.isImportDeclaration(nodes[i + 1]))
            ? ""
            : "\n")
        );
      })
  ...
  if (!context.existsFile(`${contextFilename}.ts`)) {
    context.writeFile(`${contextFilename}.ts`, getContext(filenamePrefix, filename));
  }
    ...
    ([route, verbs]: [string, PathItemObject]) => {
      Object.entries(verbs).forEach(([verb, operation]) => {
        ...
        const component: "useQuery" | "useMutate" =
          operation["x-openapi-codegen-component"] || (verb === "get" ? "useQuery" : "useMutate");
          ...
          keyManagerItems.push(
            f.createTypeLiteralNode([
              f.createPropertySignature(
                ...
                undefined,
                f.createLiteralTypeNode(f.createStringLiteral(camelizedPathParams(route)))
              ),
              f.createPropertySignature(
                undefined,
  ...
  const { nodes: usedImportsNodes, keys: usedImportsKeys } = getUsedImports(nodes, {
    ...config.schemasFiles,
    utils: utilsFilename,
  });
                    ...
                    f.createVariableDeclarationList(
                      [
                        f.createVariableDeclaration(
                          ...
                          undefined,
                          undefined,
                          f.createCallExpression(f.createIdentifier(contextHookName), undefined, [])
                        ),
                      ],
                      ts.NodeFlags.Const
                    )
      ...
      f.createVariableDeclarationList(
        [
          f.createVariableDeclaration(
            ...
            f.createArrowFunction(
              undefined,
              [f.createTypeParameterDeclaration(undefined, "TData", undefined, dataType)],
              [
                f.createParameterDeclaration(
                  undefined,
                  undefined,
                    ...
                    [
                      f.createObjectLiteralExpression(
                        [
                          f.createPropertyAssignment(
                            "queryKey",
                            f.createCallExpression(f.createIdentifier("queryKeyFn"), undefined, [
                              f.createObjectLiteralExpression([
                                f.createPropertyAssignment(
                                  "path",
                            ...
                            f.createArrowFunction(
                              ...
                              [
                                f.createParameterDeclaration(
                                  ...
                                  undefined,
                                  f.createObjectBindingPattern([
                                    f.createBindingElement(undefined, undefined, "signal"),
                                  ])
                                ),
                              ],
                              undefined,
                          ...
                          f.createSpreadAssignment(f.createIdentifier("options")),
                          f.createSpreadAssignment(f.createIdentifier("queryOptions")),
                        ],
                        true
                      ),
                    ]
...
const createUseQueryOptionsType = (dataType: ts.TypeNode, errorType: ts.TypeNode) =>
  f.createTypeReferenceNode(f.createIdentifier("Omit"), [
    f.createTypeReferenceNode(
      f.createQualifiedName(f.createIdentifier("vueQuery"), f.createIdentifier("UseQueryOptions")),
      [dataType, errorType, f.createTypeReferenceNode(f.createIdentifier("TData"), [])]
    ),
...
const createQueryImport = () =>
  f.createImportDeclaration(
    undefined,
    f.createImportClause(false, undefined, f.createNamespaceImport(f.createIdentifier("vueQuery"))),
    f.createStringLiteral("@tanstack/vue-query"),
    undefined
  );
